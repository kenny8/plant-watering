...
console.log('Loading EditBuild.jsx...');

if (!window.React || !window.AuthContext) {
  console.error('EditBuild.jsx: React or AuthContext not loaded');
...
// Reuse the same Step components from CreateBuild.jsx
...
function EditBuild({ buildId, onClose, onUpdate }) {
  console.log('EditBuild.jsx: Rendering for buildId:', buildId);
  const { token } = useAuth();
  ...
  useEffect(() => {
    console.log('EditBuild: Fetching build data for id:', buildId);
    if (!token) {
      console.error('EditBuild: No auth token available');
    ...
    window.axios
      .get(`/api/builds/${buildId}`, { headers: { Authorization: `Bearer ${token}` } })
      .then((response) => {
        console.log('EditBuild: Build data fetched:', response.data);
        setBuild(response.data);
        setLoading(false);
      })
      .catch((error) => {
        console.error('EditBuild: Fetch failed:', error.response?.data || error.message);
        setError('Ошибка при загрузке данных сборки');
        setLoading(false);
      });
  }, [buildId, token]);

  const handleSave = async () => {
    console.log('EditBuild: Updating build:', build);
    if (!token) {
      console.error('EditBuild: No auth token available');
      setError('Ошибка: Вы не авторизованы');
      return;
    ...
    try {
      const response = await window.axios.put(`/api/builds/${buildId}`, build, { 
        headers: { Authorization: `Bearer ${token}` } 
      });
      console.log('EditBuild: Update response:', response.data);
      if (onUpdate) {
        onUpdate(response.data);
      }
      onClose();
    } catch (error) {
      console.error('EditBuild: Update failed:', error.response?.data || error.message);
      setError('Ошибка при обновлении сборки');
    }
  };
  ...
  if (loading) {
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('p', { className: 'text-center' }, 'Загрузка данных сборки...')
      )
    );
  }
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement('h2', { className: 'text-2xl font-bold text-blue-600 text-center mb-4' }, 'Редактирование сборки'),
...
console.log('EditBuild.jsx: EditBuild exported:', window.EditBuild);
